classdef tree_v1 < matlab.mixin.Copyable
    ...
    methods
        ...
        function grow(obj)  
         ...
         while ~isempty(obj.branches_duplicate)
            radius_search = 0.05; %0.5 tree1_skeleton, 1 tree2_3_skeleton_modified, 1 with citiblok dist, 0.5 tree6_skeleton, 0.05 for lab vine,  0.5 for the buds test
...
%             Mdl = KDTreeSearcher(obj.leaves_all); 
            ...
            while ~found                
                ...
                if ~isempty(Idx{1})
                    ...
                    if radius_search > 0.1%2 %0.1 for tree1_2_3_skeleton, 3 tree6_skeleeton, 0.2 for the lab vine, 3 for the bud test
                        break
                    end
                end
            end            
...
%            plot3(all_positions(:,1), all_positions(:,2), all_positions(:,3), '.b');
        ...
        function draw_cylinders(obj, is_pruned)
...
%               sw = m*i+b;
              ...
              obj.branches(i).show_cylinders(new_rad, is_pruned); 
              hold on
           end
           hold off
        end  
